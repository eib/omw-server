  ...
  return function (socket) {
    ...
    socket.on('selectItem', function (data, callback) {
      ...
      var server = data.server,
        item = data.item;
      io.to(server.id).emit('selectItem', data);
      callback({ status: 'OK' });
    });

    socket.on('stopServer', function (data, callback) {
      console.log('Stopping server:', JSON.stringify(data));
      var server = data.server;
      io.to(server.id).emit('stop', {});
      callback({ status: 'OK' });
    });
